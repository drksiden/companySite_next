...
const getCategoryColor = (category: string) => {
  switch (category) {
    case "Продукция": return "bg-blue-500/10 text-blue-700 dark:bg-blue-500/20 dark:text-blue-300 border-blue-200 dark:border-blue-800";
    case "Сертификация": return "bg-emerald-500/10 text-emerald-700 dark:bg-emerald-500/20 dark:text-emerald-300 border-emerald-200 dark:border-emerald-800";
    case "Компания": return "bg-purple-500/10 text-purple-700 dark:bg-purple-500/20 dark:text-purple-300 border-purple-200 dark:border-purple-800";
    case "События": return "bg-orange-500/10 text-orange-700 dark:bg-orange-500/20 dark:text-orange-300 border-orange-200 dark:border-orange-800";
    default: return "bg-gray-500/10 text-gray-700 dark:bg-gray-500/20 dark:text-gray-300 border-gray-200 dark:border-gray-800";
  }
};
...
export function NewsClient({ featuredNews, regularNews }: NewsClientProps) {
  const onlyOneNews = !!featuredNews && regularNews.length === 0;
  const showAllButton = featuredNews && regularNews.length > 0;
  const newsCount = (featuredNews ? 1 : 0) + regularNews.length;
  const showAsFeed = newsCount >= 6;
  const autoplayPlugin = useRef(Autoplay({ delay: 4000, stopOnInteraction: true }));

  if (!featuredNews && regularNews.length === 0) return null;
        ...
        {onlyOneNews ? (
          ...
          <motion.div
            ...
            className={
              showAsFeed
                ? "flex flex-col gap-6"
                : "grid grid-cols-1 lg:grid-cols-2 gap-6 lg:gap-8 mb-12"
            }
          >
            {featuredNews && (
                          ...
                          <CarouselItem key={index}>
                            <div className="relative h-72 lg:h-96 w-full">
                              ...
                              <Image
                                ...
                                placeholder={index === 0 ? "blur" : "empty"}
                                blurDataURL={index === 0 ? "data:image/svg+xml;base64,PHN2ZyB3aWR0aD0iODAwIiBoZWlnaHQ9IjgwMCIgeG1sbnM9Imh0dHA6Ly93d3cudzMub3JnLzIwMDAvc3ZnIj48cmVjdCB3aWR0aD0iODAwIiBoZWlnaHQ9IjgwMCIgZmlsbD0iI2YzZjRmNiIvPjwvc3ZnPg==" : undefined}
                                unoptimized={image === "/images/placeholder-product.svg"}
                              />
                            </div>
                          </CarouselItem>
                        ))}
