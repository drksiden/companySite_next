  ...
  return (
    <div className={cn("space-y-4", className)}>
      ...
      <Dialog open={isFullscreen} onOpenChange={setIsFullscreen}>
        <DialogContent className="max-w-4xl w-full max-h-[90vh] h-[80vh] p-0 [&>button]:hidden bg-black/95 border-0">
          <VisuallyHidden>
          ...
          <div className="relative w-full h-full bg-transparent rounded-lg overflow-hidden">
            ...
            {/* Кнопка закрытия */}
            <button
              className="absolute top-4 right-4 h-12 w-12 rounded-full bg-white/10 hover:bg-white/20 backdrop-blur-sm text-white flex items-center justify-center z-20 transition-all duration-200 hover:scale-105 border border-white/20"
            ...
            {validImages.length > 1 && (
              <>
                <button
                  className="absolute left-4 top-1/2 transform -translate-y-1/2 h-12 w-12 rounded-full bg-white/10 hover:bg-white/20 backdrop-blur-sm text-white flex items-center justify-center z-10 transition-all duration-200 hover:scale-105 border border-white/20"
                  onClick={() =>
                    setSelectedImage(
                      selectedImage === 0
                        ? validImages.length - 1
                ...
                <button
                  className="absolute right-4 top-1/2 transform -translate-y-1/2 h-12 w-12 rounded-full bg-white/10 hover:bg-white/20 backdrop-blur-sm text-white flex items-center justify-center z-10 transition-all duration-200 hover:scale-105 border border-white/20"
                  onClick={() =>
                    setSelectedImage(
                      selectedImage === validImages.length - 1
                        ? 0
                ...
                <div className="absolute bottom-6 left-1/2 transform -translate-x-1/2 bg-white/10 backdrop-blur-sm text-white px-4 py-2 rounded-full text-sm z-10 border border-white/20">
                  {selectedImage + 1} / {validImages.length}
                </div>

                {/* Миниатюры в полноэкранном режиме */}
                <div className="absolute bottom-20 left-1/2 transform -translate-x-1/2 flex gap-2 max-w-md overflow-x-auto z-10 bg-white/5 backdrop-blur-sm rounded-full p-2 border border-white/10">
                  {validImages.map((image, index) => (
                    <button
                      key={index}
                      className={cn(
