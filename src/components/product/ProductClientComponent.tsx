...
export default function ProductClientComponent({ product, breadcrumbItems }: ProductClientComponentProps) {
  ...
  const priceInfo = useMemo(() => {
    ...
    const original = product.original_price ? formatPrice(product.original_price, product.currency_code || 'KZT') : null;
    const isSale = product.original_price != null && product.price != null && product.original_price > product.price;
    const discountPercent = isSale ? Math.round(((product.original_price! - product.price!) / product.original_price!) * 100) : 0;
            ...
            <CardHeader>
              ...
              {product.brands?.name && (
                <div className="mt-1">
                  ...
                  {product.brands.handle ? (
                    <Link href={`/brand/${product.brands.handle}`} className="text-sm text-primary hover:underline">
                      {product.brands.name}
                    </Link>
                  ) : (
                    <span className="text-sm text-foreground">{product.brands.name}</span>
                  )}
                </div>
              )}
            </CardHeader>
            ...
            <CardContent className="flex flex-col gap-5">
                ...
                <div className="flex items-center gap-3">
                  ...
                  <div className="flex items-center border rounded-md">
                    ...
                    <button
                      type="button"
                      onClick={() => {
                        const maxQuantity = inventoryQuantity > 0 ? inventoryQuantity : 999;
                        setQuantity(Math.min(maxQuantity, quantity + 1));
                      }}
                      className="px-3 py-2 hover:bg-muted transition-colors"
                      disabled={inventoryQuantity > 0 && quantity >= inventoryQuantity}
                    >
                      +
                    </button>
                  </div>
              ...
              {product.description && (
                <div className="mt-4 pt-4 border-t">
                  <h3 className="text-sm font-medium mb-2">Краткое описание:</h3>
