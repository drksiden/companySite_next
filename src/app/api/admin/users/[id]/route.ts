...
export async function PATCH(
  ...
  const requestBody: Partial<
    UserProfile & { email?: string; role?: UserRole }
  > = await req.json();
  ...
  try {
    ...
    if (profileError) {
      ...
      return NextResponse.json(
        {
          message: "Ошибка обновления профиля пользователя",
          error: profileError.message,
        },
        { status: 500 },
      );
    ...
    if (!updatedProfile) {
      return NextResponse.json(
        { message: "Профиль пользователя не найден для обновления." },
        { status: 404 },
      );
    }
    ...
    if (newRole) {
      ...
      if (authUpdateError) {
        console.error(
          `Error updating auth user role for ID ${id}:`,
          authUpdateError,
        );
    ...
    if (newEmail) {
      ...
      if (authEmailUpdateError) {
        console.error(
          `Error updating auth user email for ID ${id}:`,
          authEmailUpdateError,
        );
        return NextResponse.json(
          {
            message: `Ошибка обновления email пользователя: ${authEmailUpdateError.message}`,
          },
          { status: 500 },
        );
      }
...
export async function DELETE(
  ...
  try {
    ...
    if (authError) {
      ...
      return NextResponse.json(
        { message: "Ошибка удаления пользователя", error: authError.message },
        { status: 500 },
      );
    }
