...
export async function DELETE(req: NextRequest) {
  ...
  try {
    ...
    const deleteAuthUsersPromises = ids.map((id) =>
    ...
    if (failedDeletions.length > 0) {
      ...
      return NextResponse.json(
        {
          message:
            "Some users could not be deleted from authentication service.",
          failed_ids: failedDeletions.map((p, i) => ({
            id: ids[i],
            reason: (p as PromiseRejectedResult).reason,
          })),
        },
        { status: 500 },
      );
